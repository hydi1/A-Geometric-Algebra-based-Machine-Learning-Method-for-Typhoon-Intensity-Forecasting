...
class Net(nn.Module):
    ...
    def forward(self, wide, deep):
        ...
        for i in range(len(ahead_times)):
            deep_u=deep[:,0,:,:,:,i]#[128,4,31,31]维度的意义分别表示：batch_size,U方向的风速，压强，经度 维度，时间步长
            deep_v=deep[:,1,:,:,:,i]#[128,4,31,31]维度的意义分别表示：batch_size,V方向的风速，压强，经度 维度，时间步长

            #1、重新组织数据
            # 2、更改split 1
            # [128,32,15,15]
            deep_u_P=self.pool(F.relu(self.quaternion_conv_layer1(deep_u)))#deep_u1
            # [128,32,15,15]
            deep_u_P=self.att_block_1(deep_u_P)*deep_u_P
            # [96,32,15,15]
            deep_v_P = self.pool(F.relu(self.quaternion_conv_layer1(deep_v)))
            # [96,32,15,15]
            deep_v_P = self.att_block_1(deep_v_P) * deep_v_P#deep_v1
            #转换成复数，进行复数卷积
            # [96,32,15,15]虚数
            ...
            #进行空间注意力机制
            #[96,64,7,7]
            time_seq_1 =self.att_block_C2(time_seq_1)*time_seq_1#fuse1
            #3、更改split 2
            deep_u_P = self.pool(F.relu(self.quaternion_conv_layer2(deep_u_P)))  # [96,64,7,7]
            deep_u_P = self.att_block_2(deep_u_P) * deep_u_P  # [96,64,7,7]deep_u2
            deep_v_P = self.pool(F.relu(self.quaternion_conv_layer2(deep_v_P)))  # [96,64,7,7]
            deep_v_P = self.att_block_2(deep_v_P) * deep_v_P  # [96,64,7,7]deep_v2
            deep_uv_2 = torch.complex(deep_u_P, deep_v_P)#[96,64,7,7]complex2
            ...
            time_seq_2 =self.complex_conv3_fuse(time_seq_2)#128
...
model_name = os.path.join('24h', 'cl0102_24h.pkl')
os.makedirs('24h', exist_ok=True)


import logging
#清除已经存在的日志处理器
logging.getLogger().handlers = []
log_name = 'cl0102_24h.log'
