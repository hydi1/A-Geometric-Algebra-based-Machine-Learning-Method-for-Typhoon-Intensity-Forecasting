...
import sys
sys.path.append('D:\\project\\cliffordlayers-main')
...
def apply_complex(fr, fi, input, dtype=torch.complex64):
    #fr是conv_r,fi是conv_i,input是输入的复数张量
    #对输入的实部（input.real）应用实部卷积层 fr，得到实部卷积结果。
    #fr(input.real) - fi(input.imag)：将实部卷积结果减去虚部卷积结果，得到输出的实部。
    #fr(input.imag)：对输入的虚部（input.imag）应用实部卷积层 fr，得到实部对虚部的卷积结果。
    # fi(input.real)：对输入的实部（input.real）应用虚部卷积层 fi，得到虚部对实部的卷积结果。
    #fr(input.imag) + fi(input.real)：将上述两个结果相加，得到输出的虚部。
#我们希望输出是复数类型的张量，即使实部和虚部单独看都是实数。
#通过将实部和虚部都转换为复数类型，可以确保最终输出的张量是复数类型，这对后续的计算和处理非常重要。
#如果不进行类型转换，实部是实数类型，而虚部是复数类型，这会导致类型不匹配的问题
...
class ComplexConvTranspose2d(Module):
    ...
    def forward(self, inp):#inp是输入的复数张量 conv_tranr_r和conv_tran_i是复数卷积层
...
class ComplexConv2d(Module):
    ...
    def forward(self, inp):#inp是输入的复数张量 conv_tranr_r和conv_tran_i是复数卷积层
        output_complex = apply_complex(self.conv_r, self.conv_i, inp)
        return output_complex

# complex_conv = ComplexConv2d(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=1)
#
# # 构造输入张量
# input_real = torch.randn(96, 32,15, 15)
# input_imag = torch.randn(96, 32,15, 15)
# input_tensor=torch.complex(input_real,input_imag)
# # 调用 ComplexConv2d 的 forward 方法
# output_tensor = complex_conv(input_tensor)
