...
for ahead_time in ahead_times:
    ...
    for pressure in pressures:
        ...
        train_reanalysis_csv = pd.read_csv(
            'D:/project/Typhoon/SAF/data/ERA_Interim/' + folder + '/' + reanalysis_type + str(pressure) + '_train_31_31.csv',
            header=None)
        test_reanalysis_csv = pd.read_csv(
            'D:/project/Typhoon/SAF/data/ERA_Interim/' + folder + '/' + reanalysis_type + str(pressure) + '_test_31_31.csv',
            header=None)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv_fuse2 = nn.Conv2d(64, 128, kernel_size=3, padding=(1, 1))
        self.conv_fuse3 = nn.Conv2d(128, 256, kernel_size=3, padding=(1, 1))
    ...
    def forward(self, wide, deep):
        ...
        for i in range(len(ahead_times)):
            ...
            time_seq_1 = self.pool(F.relu(self.conv_fuse2(time_seq_1)))
            ...
            time_seq_2 = self.pool(F.relu(self.conv_fuse3(time_seq_2)))
...
model_name = os.path.join('24h', 'real_9月9.pkl')
...
logging.info(f"SAF_Net 原模型 空间注意力kernels 3*3  卷积分开定义")
